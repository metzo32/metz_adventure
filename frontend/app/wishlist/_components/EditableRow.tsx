...
export const EditableRow = ({ item, isEditing, onEdit, onSave, onCancel, onToggle, onDelete }: EditableRowProps) => {
  ...
  return (
    <tr className={`border-b border-border hover:bg-gray-50 transition ${item.is_done ? "opacity-50" : ""}`}>
      ...
      <td className="py-3 px-4">
        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${p.color}`}>{p.label}</span>
      </td>
      ...
      <td className="py-3 px-4">
        <button
          onClick={onToggle}
          className={`w-16 px-3 py-1 rounded-full text-xs font-semibold transition ${item.is_done ? "bg-green-100 text-green-600 hover:bg-green-200" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
            }`}
        >
          {item.is_done ? "완료" : "미완료"}
        </button>
